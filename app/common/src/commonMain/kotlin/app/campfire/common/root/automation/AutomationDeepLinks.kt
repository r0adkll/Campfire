...
object AutomationDeepLinks {
   ...
   */
  suspend fun applySetup(
    setup: DeepLink.Setup,
    isLoggedIn: Boolean,
  ...
  suspend fun selectLibrary(libraryName: String, libraryRepository: LibraryRepository) {
   ...
   * Root navigation destinations are listed in [rootScreenNames].
   */
  fun resolveScreen(navigate: DeepLink.Navigate): Screen? = when (navigate.screen) {
    "home" -> HomeScreen
